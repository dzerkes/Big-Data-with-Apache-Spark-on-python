...
trip_data = spark.read.format("parquet").option("header", "false").load(path_trip)
...
trip_vendors = spark     .read     .format("parquet")     .option("header", "false")     .load(path_ven)
...
trip_vendors.registerTempTable("trip_vendors")
...
res = spark.sql('select tj.vendor_id as vendor_id, tj.Distance as Distance, tj.Duration as Duration from             trip_joined as tj where tj.max_distance == tj.Distance')
